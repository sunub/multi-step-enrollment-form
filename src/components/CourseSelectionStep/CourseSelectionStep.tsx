...
export function CourseSelectionStep({ onNext }: CourseSelectionStepProps) {
  ...
  // Sync form with atom on mount or atom change (e.g. from storage)
  useEffect(() => {
  ...
  // Reactive sync: Update atom when form changes
  useEffect(() => {
    ...
    setFormAtom((prev) => {
      const nextSelectedCourse = (() => {
        ...
        if (selectedCourseSnapshot) return selectedCourseSnapshot;
